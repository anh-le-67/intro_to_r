...
ggplot(melb_housing, aes(Price))+
  geom_histogram(binwidth = 300000, color = "white", fill = "maroon") +
...
```{r}
# box plot of price and house type
...
```{r}
# descriptive statistics of price, rooms, and distance
melb_housing %>%
  summarise(mean(Price), median(Price), mean(Rooms), median(Rooms), mean(Distance), median(Distance))
```
...
lm_house <- lm(log(Price) ~ Distance + Rooms + Type, melb_housing)
```
